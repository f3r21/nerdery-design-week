/* v1 harness. The v1 component (ProviderCard.jsx/.css beside it) is frozen. Grids are the original ones (no minmax), so a long name still widens the page, as in render.png. Adds key forwarding for the deck; inside the deck the header, reference and edge cases are hidden. */
...
function App() {
  return (
    <div style={{ maxWidth: 'var(--container-app)', margin: '0 auto', padding: inDeck ? 'var(--space-2) var(--space-3)' : 'var(--space-6) var(--space-4)', display: 'grid', gap: inDeck ? 'var(--space-2)' : 'var(--space-6)' }}>
      {!inDeck && <header>
        <p className="v-eyebrow">Vello · ProviderCard v1</p>
        <h1 className="v-h2">Generated from docs and tokens only</h1>
      ...
      {!inDeck && <section style={{ display: 'grid', gap: 'var(--space-4)' }}>
